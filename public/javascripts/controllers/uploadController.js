        ...
        if($scope.primary_columns) {
          for (var ii = 0; ii < $scope.primary_columns.length; ii++) {
            ...
            var trace = {
              ...
              yaxis: 'y',
              type: 'scatter',
              name: $scope.csv_header_row[column].name.split("$$")[0].split(".")[0] + ($scope.secondary_columns.length > 0 && $scope.primary_columns.length > 0 ? ' [' + $scope.csv_header_row[column].name.split("$$")[1] + ']' : '')
            };
            data.push(trace);
          }
        }
